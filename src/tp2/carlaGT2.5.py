...
class AllMSG:
	...
	def setAllValues(self,dirDist,objKoordinaten, className,iD0):
...
def yoloTalker(allData, count):
	...
	for i in range(len(allData)):
		...
		b.obj_list[i].dimension.width = allData[i].dim.length
		...
		while True:
			...
			if step3 == 1:
				...
				if counter > 1:
						...
						if checkDetectObj(newObjDetectionName[i]) == 1:
							...
							for j in range(len(ListeObj2)):
								...
								if ("".join(ListeObj2[j].iD_Classname) == "".join(newObjDetectionName[i]) and int("".join(map(str,newObjID_Var[i]))) == ListeObj2[j].id_Klasse):
									ListeObj2[j].setAllValues(objDistance,newObjCoordinates[i],newObjDetectionName[i],newObjID_Var[i])
									idFound = True

							if idFound == False:
								ListeObj2.append(AllMSG())
								ListeObj2[i].setAllValues(objDistance,newObjCoordinates[i],newObjDetectionName[i],newObjID_Var[i])
								break
					...
					yoloTalker(ListeObj2, newObjNumber)
